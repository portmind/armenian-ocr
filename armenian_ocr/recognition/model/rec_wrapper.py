...
class Opt(object):
    ...
    def __init__(self, path):
        ...
        for i in bool_fields:
            if hasattr(self, i):
                setattr(self, i, (True if getattr(self, i) == "True" else False))
...
class ImageDataset(Dataset):
    def __init__(self, images):
        ...
        """
        self.images = [Image.fromarray(i) for i in images]  # convert to PIL Image, suitable format for the network
...
class RecWrapper(object):
    ...
    def load(
        ...
        if "CTC" in self.opt.Prediction:
            self.converter = CTCLabelConverter(self.opt.character, device=self.opt.device)
        else:
            self.converter = AttnLabelConverter(self.opt.character, device=self.opt.device)
        ...
        t_load = torch.load(os.path.join(path, model_file_name), map_location=self.device)
        self.model.load_state_dict({k[k.find(".") + 1 :]: v for k, v in t_load.items()})
    ...
    def predict(self, images):
        ...
        collate = AlignCollate(imgH=self.opt.imgH, imgW=self.opt.imgW, keep_ratio_with_pad=self.opt.PAD)
            ...
            for image_tensors, _ in image_loader:
                ...
                for i, pred in enumerate(preds_str):
                    if "Attn" in self.opt.Prediction:
                        box_prob = torch.mean(soft_prob[i][: pred.find("[s]")]).item()
                        if box_prob < 0.7:
                            pred = ""
                        else:
