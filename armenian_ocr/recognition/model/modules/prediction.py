...
class Attention(nn.Module):
    ...
    def _char_to_onehot(self, input_char, onehot_dim=38):
        ...
        one_hot = torch.zeros(
            (batch_size, onehot_dim), dtype=torch.float32
        ).to(self.device)
        one_hot = one_hot.scatter_(1, input_char.long(), 1)
        return one_hot
    ...
    def forward(self, batch_H, text, is_train=True, batch_max_length=25):
        ...
        if is_train:
            ...
            targets = torch.zeros(batch_size).to(self.device)  # [GO] token
            probs = torch.zeros((batch_size, num_steps, self.num_classes)).to(
                self.device
            )
